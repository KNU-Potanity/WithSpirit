...
- [ ] 칸마다 몬스터를 배치할 수 있도록 만들기 (2026-08-25 오늘 할 일) — (1) 인스펙터 창에서 목록을 추가할 수 있는 배열로 구현 (2) 각 요소는 x값·y값·프리팹을 지정할 수 있어야 함
...
- [ ] ⚠️ `FairySelectUI`/`FairySelectTrigger`를 실제 씬(`SampleScene.unity`)에 배치 (현재 스크립트만 있고 씬에 안 붙어있음)
- [ ] ⚠️ `FairyPool.asset`에 정령 6종 실제 등록 (현재 0개, 비어있어서 선택지가 안 뜸)
- [ ] 정령 7종 데이터 에셋의 `Category`/`FairyName`/`Description` 필드에 실제 텍스트 입력 (현재 전부 클래스 기본값)
