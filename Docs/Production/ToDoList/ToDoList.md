...
- [ ] 정령 선택/전환 로직(마우스 오른쪽 클릭) 구현 (`FairyManager.SelectNextFairy()`가 순환 선택 로직으로 이미 있음, 마우스 입력과 연결만 남음)
- [x] 정령 줄서기(대형) 기능 작업 중 (2026-08-18, `FairyManager.cs`, `정령_시스템_기획서.md` 3.6절 참고)
