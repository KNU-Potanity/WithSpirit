...
> 마지막 수정일: 2026-07-16 (6.1절 Animator Controller 현황 갱신, 6.2절 실제 적용 사례(GroundMonsterMovement.cs) 추가 — ver7)
...
### 6.2 실제 적용 사례 — 버섯(`GroundMonsterMovement.cs`) (2026-07-16)

지상 근접몹(5장)의 실제 이동 스크립트인 `Assets/_Project/Scripts/Enemy/GroundMonsterMovement.cs`에 6장의 Animator 연동을 실제로 적용하며 발견·수정한 내용입니다.

| 문제 | 원인 | 수정 |
|---|---|---|
| 이동해도 Idle에 고정 | `Animator.SetFloat("Speed", ...)`를 호출하는 코드가 아예 없었음 | `FixedUpdate`에서 매 프레임 실제 속도를 `Speed` 파라미터에 반영하도록 추가 |
| 공격 애니메이션이 재생되지 않음 | `Animator.SetTrigger("Attack")` 호출이 없었음 | 플레이어 접촉 시(`ApplyDamage`) 트리거 호출 + `DoAttack()` 코루틴으로 애니메이션이 끝날 때까지 대기 후 복귀 |
| 공격 중에도 계속 움직이거나 튕겨나감 | (1) 방향 전환 판정이 공격 판정보다 먼저 실행되어 첫 접촉 프레임에 방향이 바뀜 (2) 몬스터·플레이어 모두 Trigger가 아닌 Dynamic Rigidbody라서, 속도를 0으로 만들어도 물리 엔진이 겹친 몸체를 자동으로 밀어냄 | (1) `ApplyDamage()`를 방향 전환 판정보다 먼저 실행해 같은 프레임에 `isAttacking`이 반영되게 순서 변경 (2) 공격 중에는 `Rigidbody2D.bodyType`을 `Kinematic`으로 전환해 물리적으로 완전히 고정, 종료 후 `Dynamic`으로 복귀 |

- `Animator`는 자식 오브젝트(예: `Mushroom_Move_0`)에 붙어있고 `GroundMonsterMovement`는 부모에 붙어있으므로, `GetComponentInChildren<Animator>()`로 참조합니다.
- 이 패턴(Speed 갱신 + Attack 트리거 + 공격 중 Kinematic 고정 + 판정 순서)은 다른 지상 근접몹(나무 골렘, 식충식물)에도 동일하게 적용 예정입니다.
