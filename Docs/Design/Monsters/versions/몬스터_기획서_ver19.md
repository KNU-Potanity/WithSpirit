...
> 마지막 수정일: 2026-08-04 (7장 보스(밤 나무) 패턴 설계 추가 — ver19)
...
| 3. 밤송이 투척 | 플레이어에게 날아오는 밤송이를 투척, 닿으면 데미지 | 밤송이가 플레이어에게 닿기 전에 정령 공격으로 제거 | 높음 — 투사체 시스템 자체가 프로젝트에 아직 없음(스켈레톤 원거리 공격도 동일하게 미구현). 밤송이도 체력/피격 판정(`MonsterHealth`류)과 클릭 마커(`MonsterClickMarker`류)가 있어야 "정령 공격으로 제거"가 가능해서, 일반 투사체보다 구현량이 더 필요함 |
...
- 밤송이 투척에 필요한 투사체 시스템은 보스 전용이 아니라 스켈레톤(원거리형) 화살에도 그대로 재사용 가능한 공용 기능으로 만드는 것을 권장
