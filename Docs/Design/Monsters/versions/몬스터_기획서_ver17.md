...
> 마지막 수정일: 2026-08-04 (6.4절 근접 지상 몬스터 프리팹 추가 반영 — ver17)
...
- 씬에는 `BeeMarker` 오브젝트로 적용되어 있으며, 자식 `Bee_Idle_0`에 **Animator와 `AC_Monster_Bee.controller` 연결까지 완료된 상태**입니다 (2026-07-28 확인).
...
| 항목 | 경로 |
|---|---|
| 공통 템플릿 | `Assets/_Project/Prefabs/Entitys/Monsters/GroundMonsterPrefab.prefab` — `BoxCollider2D`+`Rigidbody2D`+`GroundMonsterMovement`+`SpriteRenderer`+`MonsterHealth` (루트) / `MonsterVisual`(SpriteRenderer+Animator) / `AttackMarker`(BoxCollider2D+MonsterClickMarker) 3-계층 구조 |
| 버섯 | `Assets/_Project/Prefabs/Entitys/Monsters/Mushroom.prefab` — `MushroomData_temp_real` 데이터 연결 |
| 나무 골렘 | `Assets/_Project/Prefabs/Entitys/Monsters/TreeGolem.prefab` — `TreeGolemData` 데이터 연결 |
| 식충식물 | `Assets/_Project/Prefabs/Entitys/Monsters/CarnivorousPlantData.prefab` — `CarnivorousPlantData` 데이터 연결 (⚠️ 파일명이 `CarnivorousPlant.prefab`이 아니라 `CarnivorousPlantData.prefab`으로 되어있어 데이터 에셋과 이름이 헷갈릴 수 있음) |
| 벌 (참고, 공중형) | `Assets/_Project/Prefabs/Entitys/Monsters/Bee.prefab` — `FloatingMonsterMovement` 사용, `MonsterVisual` 계층 없이 애니메이터가 자식에 바로 있음 |

- **폴더 위치가 기존 문서 계획과 다릅니다**: `에셋_네이밍_관리규칙.md`에는 `Assets/_Project/Prefabs/Monsters/`로 계획되어 있었는데, 실제로는 `Assets/_Project/Prefabs/Entitys/Monsters/`(중간에 `Entitys` 폴더 추가, 플레이어 프리팹도 `Prefabs/Entitys/Player/`로 함께 재배치)로 만들어졌습니다.
