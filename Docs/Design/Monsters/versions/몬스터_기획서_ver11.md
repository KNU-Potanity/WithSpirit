...
> 마지막 수정일: 2026-07-23 (5.1절에 공중형(벌) 원래 설계 의도와 실제 구현 차이점 정리 — ver11)
...
- 평상시(순찰)에는 x축을 기준으로, 자신의 움직임을 물리적으로 막는 장애물이 양쪽에 있다면 그 사이에서 왔다갔다 이동합니다.
- 카메라에 공중몹이 보였을 때, x축 기준으로 플레이어를 향해 물리적으로 못 가게 막는 장애물이 없다면 플레이어를 향해 이동합니다.
- 갈 수 없다면(장애물에 막히면) 그대로 장애물 사이에서 왔다갔다하는 순찰을 유지합니다.

**실제 구현 현황 (2026-07-23 확인, `FloatingMonsterMovement.cs`)** — 위 설계와 세부 조건이 다소 다르게 구현되어 있습니다.

지상 근접몹과 마찬가지로 **Patrol/Chase/Attack 상태 머신**이지만, 지형(플랫폼)에 구애받지 않고 자유롭게 날아다니는 방식으로 구현되어 있습니다.

| 상태 | 실제 전환 조건 | 실제 동작 |
|---|---|---|
| Patrol (기본) | 플레이어가 `chaseRange` 밖이거나 시야가 막힘 | **제자리에서 정지** (`SmoothDamp`로 현재 위치 유지 — 장애물 사이를 왔다갔다하는 순찰 이동은 아직 구현되지 않음) |
| Chase (추격) | 플레이어가 `chaseRange`(반경 거리) 안 **AND** 장애물에 막히지 않은 직선 시야(Line of Sight)가 확보됨 | `SmoothDamp`로 플레이어 위치를 향해 자유롭게 이동, 좌우 반전으로 플레이어 방향을 바라봄 |
| Attack (공격) | Chase 중 `attackRangeX`(수평)·`attackRangeY`(수직) 이내로 들어오고 쿨타임(`attackCooldown`)이 끝남 | 정지 후 데미지+넉백 적용, 쿨타임 동안 대기 후 Patrol로 복귀 |

**설계 대비 차이점**

| 항목 | 원래 설계 | 실제 구현 |
|---|---|---|
| 순찰 동작 | 장애물 사이에서 x축으로 왔다갔다 이동 | 제자리 정지만 함 (왔다갔다 이동 미구현) |
| 추격 시작 조건 | 카메라에 보였을 때 + x축 장애물 없음 | 카메라 노출 여부와 무관하게, 플레이어와의 **반경 거리**(`chaseRange`)가 기준 + 레이캐스트 시야 확인 |
| 장애물 판정 축 | x축 기준 | x/y를 모두 포함한 직선 레이캐스트(Line of Sight) |

- 지상 근접몹의 "같은 플랫폼" 판정 대신, 단순 직선 레이캐스트로 장애물 유무만 확인합니다 — 벌은 날아다니므로 낭떠러지/플랫폼 개념 자체가 적용되지 않습니다.
- 씬에는 `BeeMarker` 오브젝트로 적용되어 있으며, 자식 `Bee_Idle_0`에 `SpriteRenderer`는 있지만 **Animator는 아직 부착되지 않은 상태**입니다 (6장 애니메이션 연동 작업 필요).
- 순찰 시 왔다갔다 이동 및 카메라 노출 기반 추격 판정을 원래 설계대로 맞추려면 추가 구현이 필요합니다 (`ToDoList.md` 참고).
