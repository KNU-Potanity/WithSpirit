...
> 마지막 수정일: 2026-07-30 (정령·벌·버섯 스탯 에셋 실제 수정값 반영 — ver15)
...
| 스탯 | 수치 |
|---|---|
| 공격력 | 1 (2026-07-30 실제 에셋 `MainFairyData.asset` 값으로 수정, 이전 문서값 5) |
| 쿨타임 | 0.5초 (2026-07-30 실제 에셋 `MainFairyData.asset` 값 기준으로 수정, 이전 문서값 0.8초) |
...
| Move Speed (이동 속도) | 2 |
| Damage (공격력) | 1 |
| Cool Time (쿨타임) | 1 (이전 1.5) |
| Attack Range X (공격 범위 X) | 1.2 |
| Attack Range Y (공격 범위 Y) | 2 |
| Detection Range (감지 범위) | 3 (이전 8) |
| Knockback (넉백) | 2 |
| Attack Anim Delay (공격 애니메이션 시작 후 데미지 적용까지의 지연시간) | 0.4 |
| Hurt Flash Duration (피격 시 빨간색 깜빡임 지속시간) | 0.1 |
| Death Fade Duration (사망 시 페이드 아웃 지속시간) | 0.5 |
