...
- **ver5 변경 사항**: 씬 파일명을 실제 파일명(`SampleScene.unity`)으로 수정하고, 레벨디자인 문서 참조를 ver4로 갱신했습니다.
...
Assets/
 ...
 │   │   └─ UI/
 │   ├─ Scenes/
 │   │   └─ SampleScene.unity    # 현재 사용 중인 씬 파일명 (추후 Stage1.unity로 변경 예정)
...
### 4.4 레벨 데이터 관리
- 레벨 레이아웃(발판/장애물 배치)은 Unity **Tilemap + 씬 파일**로 관리 (코드에 하드코딩하지 않음) — 구현 완료 (`Level_Stage1_ver2` 씬 객체, 좌표는 `레벨디자인_MVP_스테이지_ver4.md` 기준)
