...
- **2026-07-28 개정**: 4.9절에 ScriptableObject 기반 데이터 관리 원칙(설정값=SO, 상태값=스크립트 필드)을 추가했습니다. 실제 몬스터 AI·플레이어 스크립트가 이 방식으로 구현되어 있습니다.
...
Assets/
 ...
 │   │   ├─ Goal/           # GoalTrigger
 │   │   ├─ Fairies/        # (예정) 정령 상호작용, 정령별 공격/플랫폼 생성 로직
...
### 4.9 ScriptableObject 기반 데이터 관리 (2026-07-28 추가)

몬스터/정령/플레이어의 **설계 수치**(체력, 이동속도, 공격력, 쿨타임, 감지범위 등)는 `ScriptableObject` 에셋으로 분리해서 관리합니다. 실제로 몬스터 AI(`GroundMonsterMovement.cs`, `FloatingMonsterMovement.cs`)와 `PlayerHealth.cs`/`PlayerJump.cs`/`PlayerGroundDetector.cs`/`FallRespawnDetector.cs`가 이 방식으로 구현되어 있습니다.

**원칙: "설정값은 ScriptableObject, 상태값은 스크립트 필드"**

| 구분 | 저장 위치 | 예시 |
|---|---|---|
| 설정값 (디자인 타임 수치, 여러 인스턴스가 같은 값을 공유해도 되는 것) | `ScriptableObject` 에셋 | 최대 체력, 이동속도, 공격력, 쿨타임, 감지범위, 점프 높이/시간 등 |
| 상태값 (런타임에 매 프레임 바뀌는 것, 인스턴스마다 달라야 하는 것) | 컴포넌트(MonoBehaviour)의 일반 필드 | 현재 체력, 현재 AI 상태(Patrol/Chase/Attack), 쿨타임 타이머, 현재 애니메이션 상태 |

⚠️ **상태값을 ScriptableObject에 넣지 않도록 주의**합니다. ScriptableObject 에셋은 하나의 파일을 여러 오브젝트가 참조로 공유하기 때문에, 여기에 현재 체력 같은 런타임 상태를 넣으면 같은 데이터 에셋을 쓰는 여러 인스턴스(예: 같은 `MushroomData.asset`를 쓰는 버섯 여러 마리)가 서로의 상태를 공유해버리는 버그가 생깁니다.

**클래스 구조**: `MonsterData`(공통 기반) → `MeleeMonsterData`/`RangedMonsterData`/`DebuffMonsterData`/`GroundMonsterData`/`FloatingMonsterData`(상속으로 확장). 정령은 `MainFairyData`/`AttackFairyData`/`PlatformFairyData`. 자세한 필드는 `스크립트_설명서.md` 참고.

**참고**: 상속 계층이 앞으로 계속 깊어지면(예: "지상+원거리" 같은 조합이 필요해지는 경우) C#은 다중 상속이 안 되므로, 상속 대신 여러 SO를 조합하는 컴포지션 구조로 전환이 필요할 수 있습니다. 지금 규모(몬스터 7종)에서는 문제 없습니다.
...
| 네임스페이스 | `BasePlatformer.Player`, `BasePlatformer.Terrain`, `BasePlatformer.Respawn`, `BasePlatformer.Goal`, `BasePlatformer.Fairies`(예정), `BasePlatformer.Monsters`(예정) |
