...
- **2026-09-14 변경 사항**: UI 폰트(NeoDunggeunmo)의 출처·라이선스를 확인해 4장 표에 추가했습니다. 이전에 임시 적용했던 DUNGGEUNMO SDF는 출처 불명으로 폐기하고 이 폰트로 교체했습니다 (`월드_스테이지_구성.md` 10.5절 참고).
...
| UI 폰트 (NeoDunggeunmo) | dalgona (달고나) | https://neodgm.dalgona.dev/ | 상업적 사용 가능한 오픈소스 폰트 (PM 확인) | 불필요 (권장) | `Assets/_Project/Fonts/neodgm.ttf`, `neodgm SDF.asset` | 시작 화면·설정 화면·정령 선택 UI 텍스트에 사용. 이전에 임시 적용했던 DUNGGEUNMO SDF(출처 불명)를 대체함 (`월드_스테이지_구성.md` 10.5절 참고) |
