...
- **ver11 변경 사항**: 체력 하트 아이콘(Flaticon 기반 `heart.png`/`heart_fill.png`)은 실제로 채택되지 않았고, 현재 하트 UI는 무관한 자체 제작 임시 에셋(`ui_heart_full/empty.png`)을 사용 중임을 확인·반영. Flaticon 항목은 미사용으로 표시하고 출처 표기도 보류로 변경.
...
| 체력 하트 아이콘 (UI, ⚠️ 현재 미사용) | Flaticon (제작자: meaicon) | https://www.flaticon.com/free-icons/pixel | Flaticon Free License | **필요 (단, 채택될 경우에만)** | `Assets/UI/Icons/Heart/` | 2026-07-16 초안으로 제작했으나 실제로는 채택되지 않음 — 현재 하트 UI는 `Assets/_Project/Art/UI/ui_heart_full.png`/`ui_heart_empty.png`(자체 제작 임시 플레이스홀더, Flaticon과 무관, 출처 표기 불필요)를 사용 중. 이 항목은 나중에 정식 채택 시를 대비한 기록용 |
| 체력 하트 아이콘 — 채우기 레이어 (파생, ⚠️ 현재 미사용) | Flaticon (제작자: meaicon)의 `heart.png`를 침식(erode)·흰색화하여 제작한 파생 이미지 | https://www.flaticon.com/free-icons/pixel | Flaticon Free License (수정 사용 허용, 출처 표기 유지) | **필요 (단, 채택될 경우에만)** | `Assets/UI/Icons/Heart/heart_fill.png` | 2-레이어(색 틴트) 방식 자체가 채택되지 않아 이 파일도 현재 미사용 (`월드_스테이지_구성.md` 6.1절 참고) |
...
- **체력 하트 아이콘(Flaticon) 관련 — ⚠️ 현재는 미사용 상태**입니다. 실제 하트 UI는 자체 제작 임시 플레이스홀더(`ui_heart_full/empty.png`)를 쓰고 있어 지금은 출처 표기가 필요 없습니다. 아래는 이 Flaticon 이미지를 나중에 정식 채택할 경우를 대비한 문구입니다.
