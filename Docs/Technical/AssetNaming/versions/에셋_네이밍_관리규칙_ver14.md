...
- 2026-07-23 개정: 정령 Animator Controller 네이밍(`AC_Fairy_이름`)과 저장 위치(`Animators/Fairies/`)를 3.2절에 추가.
- 2026-07-28 개정: ScriptableObject 데이터 에셋 네이밍(`SO_분류_이름`)과 저장 위치(`Data/{분류}/`)를 3.3.1절에 추가.
- 2026-08-04 개정: 실제 프리팹/데이터 에셋 폴더 위치가 계획(`Prefabs/Monsters/`, `Data/Monsters/`)과 다르게(`Prefabs/Entitys/Monsters/`, `ScripableObjects/Monster/`) 만들어진 것을 확인해 2장에 반영.
- 2026-08-04 개정 2: `Prefabs/Entitys/` 오타를 `Prefabs/Entities/`로 실제 폴더명까지 수정하고, 나머지 경로는 계획을 실제 구조에 맞춰 확정.
- 2026-08-18 개정: 이펙트(VFX) 애니메이션 클립/컨트롤러 네이밍(`anim_fx_이름`, `AC_FX_이름`)과 저장 위치(`Animations/Effects/`, `Animators/Effects/`)를 2장, 3.2절에 추가.
...
Assets/_Project/
 ...
 │   └─ Effects/       (신규, 이펙트 .anim 클립 — 예: 폭발)
 ├─ Animators/
 │   ├─ Monsters/       (신규, .controller — AC_Monster_이름 형식)
 │   └─ Effects/       (신규, .controller — AC_FX_이름 형식)
 ├─ Audio/
 │   ├─ SFX/
 │   └─ BGM/
 ├─ Prefabs/
 │   ├─ Entities/       (플레이어·몬스터 등 게임 내 개체 프리팹)
 │   │   ├─ Player/
 │   │   └─ Monsters/
 │   ├─ Fairies/
 │   ├─ Terrain/
 │   └─ UI/
 ├─ ScripableObjects/
 │   └─ Monster/       (몬스터 데이터 에셋 — SO_ 접두사 규칙은 3.3.1절 참고)
 ├─ Tilemaps/
 └─ Scenes/
```
...
> **2026-08-04 정리**: 계획 단계에서는 `Prefabs/Monsters/`, `Data/Monsters/`로 구상했었는데, 실제 작업하신 분이 `Prefabs/Entitys/Monsters/`, `ScripableObjects/Monster/`로 만드셨습니다. 폴더를 계획대로 옮기는 대신 **문서를 실제 구조에 맞춰 갱신**했습니다 (이미 프리팹·에셋 참조가 이 경로 기준으로 연결되어 있어, 문서만 고치는 쪽이 리스크가 적음). 단, `Entitys`는 오타(`Entities`가 맞는 철자)라서 이 부분만 실제 폴더명을 수정했습니다.
...
- 정령 공격 등에서 재생되는 **이펙트(VFX) 애니메이션**은 `Assets/_Project/Animations/Effects/`에 `anim_fx_이름.anim` 형식으로 저장하고, 전용 Animator Controller는 `Assets/_Project/Animators/Effects/`에 `AC_FX_이름.controller`로 저장합니다 (예: `anim_fx_explosion.anim` / `AC_FX_Explosion.controller`, 2026-08-18 추가).
...
- 정령은 `Assets/_Project/Animators/Fairies/`에 `AC_Fairy_이름.controller`로 저장 — 예: `AC_Fairy_Base.controller`
...
### 3.3.1 데이터 에셋 (ScriptableObject 인스턴스, 2026-07-28 추가)

형식: `SO_분류_이름.asset`
- 위치: `Assets/_Project/Data/{분류}/` (예: `Assets/_Project/Data/Monsters/`, `Assets/_Project/Data/Fairies/`)
- 예: `SO_Monster_Bee.asset`, `SO_Monster_Mushroom.asset`, `SO_Fairy_Base.asset`, `SO_Fairy_AtkExplosion.asset`
- `_temp`, `_real` 같은 임시 표시 접미사는 쓰지 않습니다 — 임시 값인지 최종 값인지 이름만으로 구분이 안 되면 나중에 헷갈립니다. 값 자체가 임시라면 파일명은 정식으로 두고 내용을 나중에 채우거나, 필요하면 문서(`ToDoList.md`)에 "임시값" 상태를 별도로 기록합니다.
- 기획(설정값)과 상태(런타임 값)를 나누는 원칙은 `기술기획서.md` 4.9절 참고.
