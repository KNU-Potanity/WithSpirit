...
Assets/_Project/
 ...
 │   └─ BGM/
 ├─ Prefabs/
 │   ├─ Entitys/       (2026-08-04 실제로 이 하위에 재배치됨 — 아래 참고)
 │   │   ├─ Player/
 │   │   └─ Monsters/       (신규)
 │   ├─ Fairies/       (신규)
 │   ├─ Terrain/
 │   └─ UI/
 ├─ Data/  (계획명. 실제로는 `ScripableObjects/Monster/`에 생성됨 — 아래 참고)
 ├─ Tilemaps/
 └─ Scenes/
```
...
⚠️ **2026-08-04 실제와 다르게 만들어진 부분**
- 몬스터/플레이어 프리팹이 계획했던 `Assets/_Project/Prefabs/Monsters/`가 아니라 **`Assets/_Project/Prefabs/Entitys/Monsters/`**(중간에 `Entitys` 폴더 추가)에 생성되어 있습니다. 플레이어 프리팹도 `Prefabs/Entitys/Player/`로 같이 재배치됨.
- 몬스터 데이터 에셋(3.3.1절)이 계획했던 `Data/Monsters/`가 아니라 **`Assets/_Project/ScripableObjects/Monster/`**에 생성되어 있습니다.
- 이 문서의 계획 경로를 실제 경로에 맞춰 갱신할지, 아니면 실제 폴더를 계획대로 옮길지 결정이 필요합니다 (`ToDoList.md` 참고).
