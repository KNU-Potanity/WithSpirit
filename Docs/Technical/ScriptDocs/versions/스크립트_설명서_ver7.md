...
| `GroundMonsterMovement.cs` | `Assets/_Project/Scripts/Enemy/GroundMonsterMovement.cs` | 지상 근접몹(버섯 등) 이동/공격 스크립트. 낭떠러지 감지 시 방향 전환, 플레이어 접촉 시 데미지+공격 애니메이션. 2026-07-16에 Animator 연동(`Speed`/`Attack` 파라미터), 공격 중 Rigidbody `Kinematic` 고정, 판정 순서 수정을 추가했습니다 (`몬스터_기획서.md` 6.2절 참고). |
...
> ⚠️ **문서와 코드가 다른 부분 (확인 필요)**
> - `캐릭터컨트롤_스펙.md` 9장은 무적 시간 **0.5초**, 깜빡임 주기 **0.15초**로 확정되어 있는데, 실제 `PlayerHealth.cs`의 기본값은 `invincibilityDuration = 10f`, `blinkInterval = 0.1f`로 되어 있어 값이 다릅니다. 코드의 Inspector 필드를 문서 값으로 맞추는 작업이 필요합니다.
> - `월드_스테이지_구성.md` 6.1절은 하트 UI를 **외곽선(`heart.png`) + 채우기(`heart_fill.png`) 2-레이어를 `color`로 틴트**하는 방식으로 설계했는데, 실제 `HeartUI.cs`는 **미리 만들어진 두 스프라이트(`fullHeartSprite`/`emptyHeartSprite`)를 통째로 교체**하는 단일 스프라이트 방식으로 구현되어 있습니다. 둘 중 하나로 설계·구현을 통일해야 합니다 (예: `fullHeartSprite`를 빨간 채우기+검정 외곽선이 합쳐진 별도 이미지로 새로 만들거나, `HeartUI.cs`를 2-레이어 방식으로 수정).
...
|---|---|---|---|
| `PlayerData.cs` | `Assets/_Project/Scripts/ScriptableObjects/PlayerData.cs` | JumpPower, MoveSpeed, Health | `캐릭터컨트롤_스펙.md` |
| `MonsterData.cs` | `Assets/_Project/Scripts/ScriptableObjects/EnemyData/MonsterData.cs` | Health, MoveSpeed, Damage, CoolTime | `스탯_기획서.md` 3.1 공통 |
| `MeleeMonsterData.cs` | `.../EnemyData/MeleeMonsterData.cs` | (MonsterData 상속, 추가 필드 없음) | `스탯_기획서.md` 3.3 근거리형 |
| `RangedMonsterData.cs` | `.../EnemyData/RangedMonsterData.cs` | AttackRange, ProjectileType(`enum ProjectileTypes` 비어있음), FiringType(`enum FiringTypes`: Straight/Arc) | `스탯_기획서.md` 3.4 원거리형 |
| `DebuffMonsterData.cs` | `.../EnemyData/DebuffMonsterData.cs` | DebuffType(`enum DebuffTypes` 비어있음), AttackRange | `스탯_기획서.md` 3.2 디버프형 |
| `MainFairyData.cs` | `.../FairyData/MainFairyData.cs` | Damage, CoolTime, PlatformSize, PlatformType(`enum PlatformTypes` 비어있음) | `스탯_기획서.md` 2.1 기본 정령 |
