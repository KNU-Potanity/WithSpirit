...
| Fairies | `FairyBase.cs` / `Fairy_*.cs` | 기본 정령 및 추가 정령 6종의 공격/플랫폼 생성 로직 |
| Terrain | `MovingPlatform.cs`, `CollapsingPlatform.cs`, `StatChangePlatform.cs` | 이동/무너짐/스탯 변화 발판 |
| Player | `PlayerHealth.cs` | 하트 기반 체력, 게임오버 처리 (※ 코드베이스에 이미 존재하는 것으로 확인되었으나 이 문서에는 아직 상세 반영 전 — 별도 확인 필요) |
