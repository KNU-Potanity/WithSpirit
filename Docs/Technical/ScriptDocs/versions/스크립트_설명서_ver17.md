...
| `BossMonsterData.cs` (신규, 2026-08-12) | `Assets/_Project/Scripts/ScriptableObjects/EnemyData/BossMonsterData.cs` | `MonsterData` 상속, 돌진 전용 필드 추가: `chargeSpeedMultiplier`, `chargePreDelay`, `chargeCooldown`, `chargeKnockback`. 이전에 몬스터_기획서에서 지적했던 "보스 전용 데이터 클래스 없음" 문제가 해결됨. |
