...
> **참고 (2026-07-16 정리 완료)**: 위 두 스크립트의 실제 동작에 맞춰 `캐릭터컨트롤_스펙.md`(무적 시간 10초/깜빡임 0.1초)와 `월드_스테이지_구성.md`(하트 UI 단일 스프라이트 교체 방식) 설계 문서를 코드 기준으로 수정했습니다. 단, `HeartUI.cs`의 `fullHeartSprite`에 넣을 빨강+외곽선이 합쳐진 완성 이미지는 아직 제작 전입니다 (`ToDoList.md` 참고).
