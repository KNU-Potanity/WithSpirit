...
| 분류 | 파일명 | 역할 요약 |
|---|---|---|
| Player | `PlayerMovement.cs` | 플레이어 좌우 이동 + 이동 경계 처리. **2026-08-06 추가**: `ApplySlowDebuff(감소율, 지속시간)` — 슬라임(`DebuffMonsterMovement.cs`)에게 맞으면 호출되어, 이동속도를 일정 비율 감소시키고 `SpriteRenderer.color`를 하늘색으로 바꿨다가 지속시간 후 원복하는 코루틴을 실행합니다 (`스탯_기획서.md` 4.1절 참고). |
