...
| 파일명 | 경로 | 역할 |
|---|---|---|
| `MonsterHealth.cs` | `Assets/_Project/Scripts/Monster/MonsterHealth.cs` | 몬스터 체력/피격/사망 처리. `TakeDamage(int)` 호출 시 `SpriteRenderer.color`를 빨간색으로 잠깐 바꿨다가 복귀(피격 연출), 체력 0 이하가 되면 `Rigidbody2D`를 Kinematic으로 전환 + `GroundMonsterMovement`와 **`FloatingMonsterMovement`(2026-08-04 추가)** 모두 비활성화 + Animator 비활성화 후 알파값 페이드 아웃하며 제거(사망 연출). 이전에는 `FloatingMonsterMovement` 비활성화가 빠져있어 공중 몬스터가 사망 후에도 계속 이동+공격하는 버그가 있었으나 수정됨 (`QA_테스트_계획서.md` 참고). `MushroomMarker`/`BeeMarker`에 실제로 부착되어 동작 중입니다. |
